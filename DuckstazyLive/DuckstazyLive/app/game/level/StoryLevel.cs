...
namespace DuckstazyLive.app.game.level
{
    public abstract class StoryLevelHud : Hud
    {
        ...
        public StoryLevelHud(Level level)
            : base(level)
        {
            BaseFont font = Application.sharedResourceMgr.GetFont(Res.FNT_INFO);
    ...
    public abstract class StoryLevel : Level
    {
        ...
        {
            if (getStage().KeyPressed(e))
                return true;

            if (base.KeyPressed(e))
                return true;

            if (e.code == KeyCode.RightShoulder || e.code == KeyCode.VK_PageDown)
            {
                nextLevel();
                return true;
            }
        ...
        protected StoryGame getController()
        {
            return (StoryGame)controller;
        }
