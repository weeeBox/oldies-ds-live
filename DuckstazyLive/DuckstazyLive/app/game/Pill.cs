    ...
    public class Pill : BaseElement
    {
        ...
        private void startEmo(int emotionType)
        {
            switch (emotionType)
            {
                case 0:
                    {
                        emoParam = 1 + (int)(RandomHelper.rnd() * 3.0);
        ...
        private void updateAlive(float dt)
        {
            ...
            if (emo && media.power > 0.5f)
            {
                if (emoCounter > 0.0f)
                {
                    ...
                    {
                        emoCounter = 0.0f;
                        emoPause = RandomHelper.rnd() * 3.0f + 2.0f;
                    }
                }
                else
                {
                    emoPause -= dt;
                    if (emoPause <= 0.0f)
                        startEmo((int)(RandomHelper.rnd() * 3.0));
                }
            }
        ...
        public void updateHook(float dt)
        {
            ...
            {
                if (MathUtils.vec2distSqr(x, y, hero.x + 27, hero.y + 20) < 10000)
                {
        ...
        public void drawEmo(Graphics g)
        {
            ...
            {
                GameTexture img = media.imgHigh;
                g.DrawImage(img, 0.5f * (width - img.GetWidth()), 0.5f * (height - img.GetHeight()), 2.0f - highCounter * 2.0f);
            }            
        }
