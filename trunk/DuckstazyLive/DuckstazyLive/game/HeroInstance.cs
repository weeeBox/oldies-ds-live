...
namespace DuckstazyLive.game
{
    public class HeroInstance
    {
        ...
        private void doStepBubble()
        {
            float px = x + 4;
            if (!flip) px = x + 50;
            hero.particles.startStepBubble(px, y + 39);
        }
        ...
        private void doLandBubbles()
        {
            ...
            while (i > 0)
            {
                ...
                hero.particles.startBubble(px, y + duck_h2, 0xff999999);
                --i;
            }
        }
        ...
        private void drawHero(Canvas dest, float x, float y, float trans)
		{
			...
			{
				if(sleep)
					hero.media.drawSleep(dest, x, y, flip, trans);
				else
					hero.media.drawDuck(dest, x, y, power, flip, wingAngle, trans);
			}
		}
        ...
        private void startSleepParticles()
        {
            ...
            while (i > 0)
            {
                ...
                hero.particles.startBubble(px, py, 0xff690c7a);
                --i;
            }
        }
        ...
        public int doToxicDamage(float cx, float cy, int dmg, int id)
		{
			...
			if(checkDive(cx, cy))
			{
                ...
                Application.sharedSoundMgr.playSound(hero.media.sndAttack);
				hero.particles.explStarsToxic(cx, cy-10, id, false);
				if(frags<3) ret = 0;
				else
				{
					if(frags<6) ret = 1;
			...
			else if(dam>0)
			{
				if(blinkTime<=0.0f)
				{
					...
					blinkTime = 12;
					hero.particles.explStarsToxic(cx, cy, id, true);
					hero.env.blanc = 1.0f;
					// media.sndToxic.play();
                    Application.sharedSoundMgr.playSound(hero.media.sndToxic);
				}
				else
				{
					hero.particles.explStarsToxic(cx, cy, id, false);
				}
